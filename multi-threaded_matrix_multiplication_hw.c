...
int main(void)
{
	...
	printf("entering struct declaration and definition\n");
	/********************************************************************************/
	...
	/*******************************************************************************/
	printf("exiting struct declaration and definition\n");
	printf("\n");

	/** Test arguments->arrayx *****************************************************/
	printf("arguments->array1:\n");
	for(int i=0; i<row_A_size; i++)
	{
		for(int j=0; j<column_A_size; j++)
		{
	...
	if(err1 !=0)
		printf("Error creating mutex, err=%s.\n", strerror(err1));
...
************************************/
	...
	for(int j=0; j<column_B_size; j++)
	{
		...
		{
			//sum += array1[row_index][k] * array2[k][j];
			sum += (*args->array1)[(args->row_index)][k] * (*args->array2)[k][j];

			//printf("testing: j=%d, k=%d array1[][]= %d\n", j, k, (*args->array1)[args->row_index][k]);
		}
	
		//array3[args->row_index][j] = sum;
